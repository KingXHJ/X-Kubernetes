...
export Kregion='eastus'
export Ksnet='kube'
export Kvnetcidr='10.144.0.0/16'
export Ksnetcidr='10.144.0.0/24'
export Kvmimage='Ubuntu2204'
export Kvmcontrolplanesize='Standard_B2s'
export Kvmworkersize='Standard_B1s'
export Kuser='kubeadmin'


echo "\nStart to create Resource Group!!!\n"

az group create -n $Kname -l $Kregion

echo "\nFinished to create Resource Group!!!\n"


# Network
echo "\nStart to create VNET!!!\n"
...
echo "\nFinished to create NSG!!!\n"


echo "\nStart to create NSG RULE!!!\n"
...
echo "\nFinished to create NSG RULE!!!\n"


echo "\nStart to Update VNET Subnet!!!\n"
...
echo "\nFinished to Update VNET Subnet!!!\n"


# Deploy Kubernetes control plane node VM
echo "\nStart to create VM kube-controlplane!!!\n"
...
--size $Kvmcontrolplanesize \
--nsg $Kname \
--public-ip-sku Standard

echo "\nFinished to create VM kube-controlplane!!!\n"


# Deploy Kubernetes worker node VM
echo "\nStart to create VM kube-worker!!!\n"
...
echo "\nFinished to create VM kube-worker!!!\n"
```
...
sudo ssh -i ~/.ssh/id_rsa_azure $Kuser@$Kcontrolplaneip
```
...
sudo apt -y install ebtables ethtool apt-transport-https ca-certificates curl gnupg docker.io
...
#ssh $Kuser@$Kworkerip
sudo ssh -i ~/.ssh/id_rsa_azure $Kuser@$Kworkerip
```
